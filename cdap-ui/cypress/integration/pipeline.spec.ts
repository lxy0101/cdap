...
describe('Creating a pipeline', () => {
  ...
  it('should run or not run Preview based on user action on postrun action modal', () => {
    ...
    cy.get('[data-cy="run-preview-btn"]').should('be.visible');
    ...
    // Close banner remindinig user to add source and sink
    cy.get('[data-cy="valium-banner-hydrator"]').contains(PREVIEW_FAILED_BANNER_MSG);
    cy.get('.close').click();
    cy.get('[data-cy="toggle-preview-btn"]')
      .should('be.visible')
      .click();
    cy.get('[data-cy="schedule-pipeline-btn"]').should('be.visible');
  });
